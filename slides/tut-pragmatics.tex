...
\input{tutorial}
\title{Tutorial Four — Implicature, Politeness, and Scalar Meaning}
...
\\ Why might it trigger an implicature based on a Horn scale?
  \abox{\textit{Believe} is part of a scale like \horn{certain, know, believe, think}.
...
\item What is the scalar implicature in this exchange?
  ...
  \end{description}
  \abox{``Tried’’ belongs to a scale like \horn{succeed, manage, try}.
  Choosing ``tried’’ implicates that the stronger outcomes (``managed’’ or ``succeeded’’) did not happen —
  because a cooperative speaker would have chosen the stronger term if it were true.
  This implies failure or at least incomplete success, even though the speaker avoids saying so directly.}

  How could you cancel this implicature without contradiction?
  \abox{You can add a continuation that explicitly denies it: \textit{I tried to finish the report — and actually, I managed to!}
  The implicature of non-success disappears without any logical contradiction, showing it is \textbf{defeasible}
  (unlike an entailment, which cannot be cancelled).}
...
\\  It is an indirect request, which softens it a little.}
 \item Which face is threatened in this interaction?
...
\item Explain what is going on \href{https://www.youtube.com/watch?v=lLR-V2S0DC8}{here}:
  ...
  \begin{description}
    ...
    \end{description}
 \abox{The clerk answers literally but not cooperatively: Clouseau clearly meant ``the dog next to you,''
  not just any dog. This is a failure of Relation — the answer is not relevant to the intended question.
  One could also argue the clerk is being \textbf{deliberately uncooperative}, exploiting the literal
  meaning of \textit{your} to avoid responsibility. Grice's Cooperative Principle assumes good faith,
  but not all speakers are as cooperative as they should be.}
