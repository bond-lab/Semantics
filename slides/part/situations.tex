...
\begin{frame}[allowframebreaks]{Semantic Roles}
...
\item  \txx{SOURCE}
 ...
 \item  Typically indexed by \lex{from} PP in English
 \end{itemize}
  \begin{exe}
...
\begin{frame}[allowframebreaks]{PropBank Roles}
...
\item For the core roles, meaning is defined per verb sense, with typical values given below \\
  \medskip
  \begin{tabular}{lll}
    Role & Description & Example \\ \hline
    ARG0 & agent          & \\
    ARG1 & patient   & \\
    ...
    ARG3 & starting point   & \\
    ARG4 & ending point   & \\
    ARGA & secondary agent & \eng{\ul{Kim}$_A$ \ull{walked} \ul{her dog}$_0$}
  \end{tabular}
\framebreak  
