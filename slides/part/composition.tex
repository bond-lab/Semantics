...
\begin{center}
  \scalebox{1}{
    \begin{tikzpicture}
      ...
      \endscope
      % outline
      \draw (0,0) circle (2) node [text=black,left,align=center] {\lex{big}\\  \lex{things}} 
      (2,0) circle (2) node [text=black,right] {\lex{heads}};
      \node[align=center] at (1,0) { \lex{big}\\  \lex{heads}};
    \end{tikzpicture}} 
\end{center}
...
\item This is the simplest form of composition
  ...
  \item \eng{a big mouse} is smaller than \eng{a small elephant}
  \end{itemize}
\end{itemize}
...
\item Word combinations (\txx{multi-word expressions}) can pick up new
  meanings
  \begin{itemize}
  \item \eng{They have a \ul{big head}} ``They are vain''
  \item \eng{They are a \ul{red head}} ``They have red hair''
  \end{itemize}
