...
\begin{itemize}
\item We can represent words as a vector of numbers
...
\begin{frame}{We can learn these from raw text}
  \begin{itemize}
  \item \txx{Static embeddings} (word2vec, GloVe): trained to predict
    context words from a centre word (\txx{Skip-gram}), or the centre
    word from its context words (\txx{CBOW}); each word gets one fixed vector
  \item \txx{Contextual embeddings} (BERT): a neural encoder trained to
    predict \emph{masked} tokens in a sentence --- the same word gets a
    different vector depending on its context
  \item \txx{Generative LLMs} (GPT family): trained to predict the
    \emph{next} token given all previous tokens; context-sensitive
    embeddings emerge as a by-product of this objective
...
\begin{frame}{Meaning and distribution}
  ...
  \item  “Die Bedeutung eines Wortes liegt in seinem Gebrauch.”
    \\ {}[The meaning of a word is its use in the language]
     \\ \  \hfill — Ludwig Wittgenstein (1953)
...
\begin{frame}{What is the meaning of “bardiwac”?}
  ...
  \item[~] it  probably appears in the same contexts as \lex{red wine}, \lex{port} \ldots
  \end{itemize}  
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
  \item A very nice word embedding tutorial: \url{https://www.cs.cmu.edu/~dst/WordEmbeddingDemo/tutorial.html} from Carnegie Mellon University.
  \end{itemize}
