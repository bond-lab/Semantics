...
\begin{frame}{Possessive Idiomatic Constructions}

  \begin{itemize}
  \item Some English idioms contain a variable
  \item  \lex{wrack one's brains}
``think hard''
    \begin{exe}
      \ex I wrack my brains
      \ex You wrack your brains
      \ex Kim wracks their brains
    \end{exe}
  \item The possessor of the `brains' must be the subject
  \item The equivalent in Japanese does not: 知恵を 絞る \lex{chie-wo shiboru} ``think hard: lit., squeeze knowledge''
  \end{itemize}
...
[ \avmspan{Idiom entry}\\
 ...
 (take back what one has said)\\
    $^{*}$N\textsubscript{1} & \DF{S: (n) statement (a message that is declared)}\\ 
...
[\avmspan{Idiom entry}\\
    Index form & \lex{twiddle one's thumbs}\\	
     ...
     V\textsubscript{1} & S: (v) twiddle, fiddle with (manipulate)\\
     N\textsubscript{1} & S: (n) thumb, pollex (the thick short finger)\\
...
\begin{frame}[allowframebreaks]{Corpus Examples}

These can be modified, and the internal modifier can behave like an adverb (modifying the paraphrased meaning).
...
They can be inside already complex sentences.
