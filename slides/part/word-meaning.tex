...
\item If you speak another language, then you can use that to
  ...
  \begin{itemize}
  \item \lex{leave, entrust [something, to someone]} = 預ける \jpn{azukeru}
  \item \lex{get out, go out, leave, exit [somewhere]} = 去る \jpn{saru}
  \item \lex{leave, will, bequeath [something, to someone]} = 遺す \jpn{nokosu}
  \item \lex{leave [something, somewhere]} = 残す \jpn{nokosu}
  \item \lex{remain} = 残る \jpn{nokoru}
  \end{itemize}
  \begin{taskb}
