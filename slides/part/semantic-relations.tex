...
\item But a given word can have multiple interpretations
  \begin{itemize}
...
\begin{frame}{Not everything is lexicalized}

Lexicalization is the process by which new words, having gained widespread usage, enter the lexicon. \\[2ex]
...
%      pig & hog & sow & piglet\\
    ...
    \lex{snake} is not ambiguous it is \txx{vague}: the meaning is underspecified. 
...
\begin{frame}{Semantic Relations as Sets ($p \subset q$ and $p \sim q$)}
...
\item There are many, many more lexical relations advocated by various
  theories including:
  \begin{itemize}
  \item Troponymy/hypernymy (cf.\ \eng{walk} vs.\ \eng{lollop} ``walk with a clumsy bounding stride'')
