...
\item Classify following Czech idioms as
  ...
  \end{enumerate}
  \begin{exe}
    \ex \eng{házet klacky pod nohy} ``throw sticks under feet' \abox{Fixed Expression}
    \ex \eng{mít máslo na hlavě} ``have butter on head'' \abox{Semi-Fixed Expression}
    \ex \eng{vzít roha} ``run away''\abox{Decomposable Expression}
    \ex \eng{tak říkajíc} ``so to speak''\abox{Decomposable Expression}
    \ex \eng{sůl a pepř} ``salt and pepper''\abox{Institutionalized Phrases}
    \ex \eng{zuby nehty} ``tooth and nail''\abox{Fixed Expression}
    \ex \eng{ztratit hlavu} ``lose head''\abox{Decomposable Expression}
    \ex \eng{otevřít někomu oči} ``open someone's eyes''\abox{Decomposable Expression}
    \ex \eng{v průběhu} ``in progress'' (during)\abox{Fixed Expression}
    \ex \eng{na koleně} ``on knee'' (improvised)\abox{Fixed Expression}
  \end{exe}
\end{enumerate}
