...
\item Often words are linked by more or less systematic relations, sometimes morphologically marked
  \begin{itemize}
  \item beauty/beautiful/beautify
  \item cute/cuteness
  \end{itemize}
\end{itemize}
...
\item Japanese (suffix distinguishes person/machine)
  ...
  \begin{itemize}
  \item \jpn{運転する} → \jpn{運転者} \jpn[driver]{unten-sha}
  \item \jpn{計算する} → \jpn{計算者} \jpn{計算機} \jpn[computer]{keisan-sha/ki}
  \item \jpn{研究する} → \jpn{研究者} \jpn{研究員} \jpn[researcher]{kenkyuu-sha/in}
  \item \jpn{読む} → \jpn{読み手} \jpn{読者} \jpn[reader]{yomite/dokusha}
  \end{itemize}
